...
function display_priority_r_t($name, $ip, $players, $maxplayers, $onlinestatus, $port, $customname, $whitelist, $id, $serverversion)
{
	preg_match('/\d+(\.\d+)+/', $serverversion, $matches);
	$serverversion = $matches[0];
	...
	echo "
          ...
          <td><strong>$players/$maxplayers</strong></td><td>";
	if($onlinestatus == "Online")
		echo '<span class="label label-success">';
	else if($onlinestatus == "Offline")
		echo '<span class="label label-important">';
